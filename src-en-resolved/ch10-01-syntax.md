...
<span class="caption">لیستینگ ۱۰-۴: Two functions that differ only in their names and in the types in their signatures</span>
...
<span class="caption">لیستینگ ۱۰-۵: The `largest` function using generic type parameters; this doesn’t compile yet</span>
...
<span class="caption">لیستینگ ۱۰-۶: A `Point<T>` struct that holds `x` and `y` values of type `T`</span>
...
<span class="caption">لیستینگ ۱۰-۷: The fields `x` and `y` must be the same type because both have the same generic data type `T`.</span>
...
<span class="caption">لیستینگ ۱۰-۸: A `Point<T, U>` generic over two types so that `x` and `y` can be values of different types</span>
...
<span class="caption">لیستینگ ۱۰-۹: Implementing a method named `x` on the `Point<T>` struct that will return a reference to the `x` field of type `T`</span>
...
<span class="caption">لیستینگ ۱۰-۱۰: An `impl` block that only applies to a struct with a particular concrete type for the generic type parameter `T`</span>
...
<span class="caption">لیستینگ ۱۰-۱۱: A method that uses generic types that are different from its struct’s definition</span>
