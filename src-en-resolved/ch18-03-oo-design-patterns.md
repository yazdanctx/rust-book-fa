...
<span class="caption">لیستینگ ۱۸-۱۱: Code that demonstrates the desired behavior we want our `blog` crate to have</span>
...
<span class="caption">لیستینگ ۱۸-۱۲: Definition of a `Post` struct and a `new` function that creates a new `Post` instance, a `State` trait, and a `Draft` struct</span>
...
<span class="caption">لیستینگ ۱۸-۱۳: Implementing the `add_text` method to add text to a post’s `content`</span>
...
<span class="caption">لیستینگ ۱۸-۱۴: Adding a placeholder implementation for the `content` method on `Post` that always returns an empty string slice</span>
...
<span class="caption">لیستینگ ۱۸-۱۵: Implementing `request_review` methods on `Post` and the `State` trait</span>
...
<span class="caption">لیستینگ ۱۸-۱۶: Implementing the `approve` method on `Post` and the `State` trait</span>
...
<span class="caption">لیستینگ ۱۸-۱۷: Updating the `content` method on `Post` to delegate to a `content` method on `State`</span>
...
<span class="caption">لیستینگ ۱۸-۱۸: Adding the `content` method to the `State` trait</span>
...
<span class="caption">لیستینگ ۱۸-۱۹: A `Post` with a `content` method and a `DraftPost` without a `content` method</span>
...
<span class="caption">لیستینگ ۱۸-۲۰: A `PendingReviewPost` that gets created by calling `request_review` on `DraftPost` and an `approve` method that turns a `PendingReviewPost` into a published `Post`</span>
...
<span class="caption">لیستینگ ۱۸-۲۱: Modifications to `main` to use the new implementation of the blog post workflow</span>
