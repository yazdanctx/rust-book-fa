...
<span class="caption">لیستینگ ۱۵-۱: Storing an `i32` value on the heap using a box</span>
...
<span class="caption">لیستینگ ۱۵-۲: The first attempt at defining an enum to represent a cons list data structure of `i32` values</span>
...
<span class="caption">لیستینگ ۱۵-۳: Using the `List` enum to store the list `1, 2, 3`</span>
...
<span class="caption">لیستینگ ۱۵-۴: The error we get when attempting to define a recursive enum</span>
...
<span class="caption">لیستینگ ۱۵-۵: The definition of `List` that uses `Box<T>` in order to have a known size</span>
