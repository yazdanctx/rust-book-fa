...
<span class="caption">لیستینگ ۱۷-۱: Defining an async function to get the title element from an HTML page</span>
...
<span class="caption">لیستینگ ۱۷-۲: Chaining with the `await` keyword</span>
...
<span class="caption">لیستینگ ۱۷-۳: Calling the `page_title` function from `main` with a user-supplied argument</span>
...
<span class="caption">لیستینگ ۱۷-۴: Awaiting an async block with `trpl::block_on`</span>
...
<span class="caption">لیستینگ ۱۷-۵: Calling `page_title` for two URLs to see which returns first</span>
