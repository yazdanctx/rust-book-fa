...
<span class="caption">لیستینگ ۶-۶: A `match` that only cares about executing code when the value is `Some`</span>
...
<span class="caption">لیستینگ ۶-۷: Checking whether a state existed in 1900 by using conditionals nested inside an `if let`</span>
...
<span class="caption">لیستینگ ۶-۸: Using `if let` to produce a value or return early</span>
...
<span class="caption">لیستینگ ۶-۹: Using `let...else` to clarify the flow through the function</span>
