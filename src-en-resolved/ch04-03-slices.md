...
<span class="caption">لیستینگ ۴-۷: The `first_word` function that returns a byte index value into the `String` parameter</span>
...
<span class="caption">لیستینگ ۴-۸: Storing the result from calling the `first_word` function and then changing the `String` contents</span>
...
<span class="caption">لیستینگ ۴-۹: Improving the `first_word` function by using a string slice for the type of the `s` parameter</span>
