...
<span class="caption">لیستینگ ۲۰-۱: Creating raw pointers with the raw borrow operators</span>
...
<span class="caption">لیستینگ ۲۰-۲: Creating a raw pointer to an arbitrary memory address</span>
...
<span class="caption">لیستینگ ۲۰-۳: Dereferencing raw pointers within an `unsafe` block</span>
...
<span class="caption">لیستینگ ۲۰-۴: Using the safe `split_at_mut` function</span>
...
<span class="caption">لیستینگ ۲۰-۵: An attempted implementation of `split_at_mut` using only safe Rust</span>
...
<span class="caption">لیستینگ ۲۰-۶: Using unsafe code in the implementation of the `split_at_mut` function</span>
...
<span class="caption">لیستینگ ۲۰-۷: Creating a slice from an arbitrary memory location</span>
...
<span class="caption">لیستینگ ۲۰-۸: Declaring and calling an `extern` function defined in another language</span>
...
<span class="caption">لیستینگ ۲۰-۹: Explicitly marking a function as `safe` within an `unsafe extern` block and calling it safely</span>
...
<span class="caption">لیستینگ ۲۰-۱۰: Defining and using an immutable static variable</span>

Static variables are similar to constants, which we discussed in the
[“Declaring Constants”][constants] section in Chapter 3. The
...
<span class="caption">لیستینگ ۲۰-۱۱: Reading from or writing to a mutable static variable is unsafe.</span>
...
<span class="caption">لیستینگ ۲۰-۱۲: Defining and implementing an unsafe trait</span>

By using `unsafe impl`, we’re promising that we’ll uphold the invariants that
the compiler can’t verify.

As an example, recall the `Send` and `Sync` marker traits we discussed in the
[“Extensible Concurrency with `Send` and `Sync`”][send-and-sync]
