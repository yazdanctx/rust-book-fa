...
<span class="caption">لیستینگ ۱۷-۶: Creating a new task to print one thing while the main task prints something else</span>
...
<span class="caption">لیستینگ ۱۷-۷: Using `await` with a join handle to run a task to completion</span>
...
<span class="caption">لیستینگ ۱۷-۸: Using `trpl::join` to await two anonymous futures</span>
...
<span class="caption">لیستینگ ۱۷-۹: Creating an async channel and assigning the two halves to `tx` and `rx`</span>
...
<span class="caption">لیستینگ ۱۷-۱۰: Sending and receiving multiple messages over the async channel and sleeping with an `await` between each message</span>
...
let` and `let...else`”][if-let] section in Chapter 6. The loop
...
<span class="caption">لیستینگ ۱۷-۱۱: Separating `send` and `recv` into their own `async` blocks and awaiting the futures for those blocks</span>
...
<span class="caption">لیستینگ ۱۷-۱۲: A revision of the code from Listing 17-11 that correctly shuts down when complete</span>
...
<span class="caption">لیستینگ ۱۷-۱۳: Using multiple producers with async blocks</span>
