...
<span class="caption">لیستینگ ۱۰-۱۶: An attempt to use a reference whose value has gone out of scope</span>
...
<span class="caption">لیستینگ ۱۰-۱۷: Annotations of the lifetimes of `r` and `x`, named `'a` and `'b`, respectively</span>
...
<span class="caption">لیستینگ ۱۰-۱۸: A valid reference because the data has a longer lifetime than the reference</span>
...
<span class="caption">لیستینگ ۱۰-۱۹: A `main` function that calls the `longest` function to find the longer of two string slices</span>
...
<span class="caption">لیستینگ ۱۰-۲۰: An implementation of the `longest` function that returns the longer of two string slices but does not yet compile</span>
...
<span class="caption">لیستینگ ۱۰-۲۱: The `longest` function definition specifying that all the references in the signature must have the same lifetime `'a`</span>
...
<span class="caption">لیستینگ ۱۰-۲۲: Using the `longest` function with references to `String` values that have different concrete lifetimes</span>
...
<span class="caption">لیستینگ ۱۰-۲۳: Attempting to use `result` after `string2` has gone out of scope</span>
...
<span class="caption">لیستینگ ۱۰-۲۴: A struct that holds a reference, requiring a lifetime annotation</span>
...
<span class="caption">لیستینگ ۱۰-۲۵: A function we defined in Listing 4-9 that compiled without lifetime annotations, even though the parameter and return type are references</span>
