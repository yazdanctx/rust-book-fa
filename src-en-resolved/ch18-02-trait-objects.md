...
`Sized` Trait”][dynamically-sized] in Chapter 20.) We can use
...
<span class="caption">لیستینگ ۱۸-۳: Definition of the `Draw` trait</span>
...
<span class="caption">لیستینگ ۱۸-۴: Definition of the `Screen` struct with a `components` field holding a vector of trait objects that implement the `Draw` trait</span>
...
<span class="caption">لیستینگ ۱۸-۵: A `run` method on `Screen` that calls the `draw` method on each component</span>
...
<span class="caption">لیستینگ ۱۸-۶: An alternate implementation of the `Screen` struct and its `run` method using generics and trait bounds</span>
...
<span class="caption">لیستینگ ۱۸-۷: A `Button` struct that implements the `Draw` trait</span>
...
<span class="caption">لیستینگ ۱۸-۸: Another crate using `gui` and implementing the `Draw` trait on a `SelectBox` struct</span>
...
<span class="caption">لیستینگ ۱۸-۹: Using trait objects to store values of different types that implement the same trait</span>
...
<span class="caption">لیستینگ ۱۸-۱۰: Attempting to use a type that doesn’t implement the trait object’s trait</span>
...
[in the reference][dyn-compatibility]. However, we did get extra
