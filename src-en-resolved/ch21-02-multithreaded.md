...
<span class="caption">لیستینگ ۲۱-۱۰: Simulating a slow request by sleeping for five seconds</span>
...
<span class="caption">لیستینگ ۲۱-۱۱: Spawning a new thread for each stream</span>
...
<span class="caption">لیستینگ ۲۱-۱۲: Our ideal `ThreadPool` interface</span>
...
<span class="caption">لیستینگ ۲۱-۱۳: Implementing `ThreadPool::new` to panic if `size` is zero</span>
...
<span class="caption">لیستینگ ۲۱-۱۴: Creating a vector for `ThreadPool` to hold the threads</span>
...
<span class="caption">لیستینگ ۲۱-۱۵: Modifying `ThreadPool` to hold `Worker` instances instead of holding threads directly</span>
...
<span class="caption">لیستینگ ۲۱-۱۶: Modifying `ThreadPool` to store the sender of a channel that transmits `Job` instances</span>
...
<span class="caption">لیستینگ ۲۱-۱۷: Passing the receiver to each `Worker`</span>
...
<span class="caption">لیستینگ ۲۱-۱۸: Sharing the receiver among the `Worker` instances using `Arc` and `Mutex`</span>
...
<span class="caption">لیستینگ ۲۱-۱۹: Creating a `Job` type alias for a `Box` that holds each closure and then sending the job down the channel</span>
...
<span class="caption">لیستینگ ۲۱-۲۰: Receiving and executing the jobs in the `Worker` instance’s thread</span>
...
<span class="caption">لیستینگ ۲۱-۲۱: An alternative implementation of `Worker::new` using `while let`</span>
