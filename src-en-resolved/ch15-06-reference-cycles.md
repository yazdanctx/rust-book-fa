...
<span class="caption">لیستینگ ۱۵-۲۵: A cons list definition that holds a `RefCell<T>` so that we can modify what a `Cons` variant is referring to</span>
...
<span class="caption">لیستینگ ۱۵-۲۶: Creating a reference cycle of two `List` values pointing to each other</span>
...
<span class="caption">لیستینگ ۱۵-۲۷: Creating a `leaf` node with no children and a `branch` node with `leaf` as one of its children</span>
...
<span class="caption">لیستینگ ۱۵-۲۸: A `leaf` node with a weak reference to its parent node, `branch`</span>
...
<span class="caption">لیستینگ ۱۵-۲۹: Creating `branch` in an inner scope and examining strong and weak reference counts</span>
