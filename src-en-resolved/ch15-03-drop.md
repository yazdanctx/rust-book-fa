...
<span class="caption">لیستینگ ۱۵-۱۴: A `CustomSmartPointer` struct that implements the `Drop` trait where we would put our cleanup code</span>
...
<span class="caption">لیستینگ ۱۵-۱۵: Attempting to call the `drop` method from the `Drop` trait manually to clean up early</span>
...
<span class="caption">لیستینگ ۱۵-۱۶: Calling `std::mem::drop` to explicitly drop a value before it goes out of scope</span>
