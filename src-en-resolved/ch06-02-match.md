...
<span class="caption">لیستینگ ۶-۳: An enum and a `match` expression that has the variants of the enum as its patterns</span>
...
<span class="caption">لیستینگ ۶-۴: A `Coin` enum in which the `Quarter` variant also holds a `UsState` value</span>
...
<span class="caption">لیستینگ ۶-۵: A function that uses a `match` expression on an `Option<i32>`</span>
