...
<span class="caption">لیستینگ ۱۵-۱۷: Demonstrating that we’re not allowed to have two lists using `Box<T>` that try to share ownership of a third list</span>
...
<span class="caption">لیستینگ ۱۵-۱۸: A definition of `List` that uses `Rc<T>`</span>
...
<span class="caption">لیستینگ ۱۵-۱۹: Printing the reference count</span>
...
Reference Cycles Using `Weak<T>`”][preventing-ref-cycles].
