...
<span class="caption">لیستینگ ۱۵-۲۰: A library to keep track of how close a value is to a maximum value and warn when the value is at certain levels</span>
...
<span class="caption">لیستینگ ۱۵-۲۱: An attempt to implement a `MockMessenger` that isn’t allowed by the borrow checker</span>
...
<span class="caption">لیستینگ ۱۵-۲۲: Using `RefCell<T>` to mutate an inner value while the outer value is considered immutable</span>
...
<span class="caption">لیستینگ ۱۵-۲۳: Creating two mutable references in the same scope to see that `RefCell<T>` will panic</span>
...
<span class="caption">لیستینگ ۱۵-۲۴: Using `Rc<RefCell<i32>>` to create a `List` that we can mutate</span>
