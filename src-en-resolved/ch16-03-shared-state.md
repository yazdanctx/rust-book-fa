...
<span class="caption">لیستینگ ۱۶-۱۲: Exploring the API of `Mutex<T>` in a single-threaded context for simplicity</span>
...
<span class="caption">لیستینگ ۱۶-۱۳: Ten threads, each incrementing a counter guarded by a `Mutex<T>`</span>
...
<span class="caption">لیستینگ ۱۶-۱۴: Attempting to use `Rc<T>` to allow multiple threads to own the `Mutex<T>`</span>
...
<span class="caption">لیستینگ ۱۶-۱۵: Using an `Arc<T>` to wrap the `Mutex<T>` to be able to share ownership across multiple threads</span>
