...
<span class="caption">لیستینگ ۱۶-۱: Creating a new thread to print one thing while the main thread prints something else</span>
...
<span class="caption">لیستینگ ۱۶-۲: Saving a `JoinHandle<T>` from `thread::spawn` to guarantee the thread is run to completion</span>
...
<span class="caption">لیستینگ ۱۶-۳: Attempting to use a vector created by the main thread in another thread</span>
...
<span class="caption">لیستینگ ۱۶-۴: A thread with a closure that attempts to capture a reference to `v` from a main thread that drops `v`</span>
...
<span class="caption">لیستینگ ۱۶-۵: Using the `move` keyword to force a closure to take ownership of the values it uses</span>
