...
<span class="caption">لیستینگ ۱۵-۶: Using the dereference operator to follow a reference to an `i32` value</span>
...
<span class="caption">لیستینگ ۱۵-۷: Using the dereference operator on a `Box<i32>`</span>
...
<span class="caption">لیستینگ ۱۵-۸: Defining a `MyBox<T>` type</span>
...
<span class="caption">لیستینگ ۱۵-۹: Attempting to use `MyBox<T>` in the same way we used references and `Box<T>`</span>
...
<span class="caption">لیستینگ ۱۵-۱۰: Implementing `Deref` on `MyBox<T>`</span>
...
recall from [“Creating Different Types with Tuple Structs”][tuple-structs] in Chapter 5 that `.0` accesses the first value in a tuple struct.
...
<span class="caption">لیستینگ ۱۵-۱۱: A `hello` function that has the parameter `name` of type `&str`</span>
...
<span class="caption">لیستینگ ۱۵-۱۲: Calling `hello` with a reference to a `MyBox<String>` value, which works because of deref coercion</span>
...
<span class="caption">لیستینگ ۱۵-۱۳: The code we would have to write if Rust didn’t have deref coercion</span>
