...
<span class="caption">لیستینگ ۲۰-۱۳: The definition of the `Iterator` trait that has an associated type `Item`</span>
...
<span class="caption">لیستینگ ۲۰-۱۴: A hypothetical definition of the `Iterator` trait using generics</span>
...
<span class="caption">لیستینگ ۲۰-۱۵: Implementing the `Add` trait to overload the `+` operator for `Point` instances</span>
...
<span class="caption">لیستینگ ۲۰-۱۶: Implementing the `Add` trait on `Millimeters` to add `Millimeters` and `Meters`</span>
...
<span class="caption">لیستینگ ۲۰-۱۷: Two traits are defined to have a `fly` method and are implemented on the `Human` type, and a `fly` method is implemented on `Human` directly.</span>
...
<span class="caption">لیستینگ ۲۰-۱۸: Calling `fly` on an instance of `Human`</span>
...
<span class="caption">لیستینگ ۲۰-۱۹: Specifying which trait’s `fly` method we want to call</span>
...
<span class="caption">لیستینگ ۲۰-۲۰: A trait with an associated function and a type with an associated function of the same name that also implements the trait</span>
...
<span class="caption">لیستینگ ۲۰-۲۱: Attempting to call the `baby_name` function from the `Animal` trait, but Rust doesn’t know which implementation to use</span>
...
<span class="caption">لیستینگ ۲۰-۲۲: Using fully qualified syntax to specify that we want to call the `baby_name` function from the `Animal` trait as implemented on `Dog`</span>
...
<span class="caption">لیستینگ ۲۰-۲۳: Implementing the `OutlinePrint` trait that requires the functionality from `Display`</span>
...
<span class="caption">لیستینگ ۲۰-۲۴: Creating a `Wrapper` type around `Vec<String>` to implement `Display`</span>
