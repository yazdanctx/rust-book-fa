...
<span class="caption">لیستینگ ۱۰-۱۲: A `Summary` trait that consists of the behavior provided by a `summarize` method</span>
...
<span class="caption">لیستینگ ۱۰-۱۳: Implementing the `Summary` trait on the `NewsArticle` and `SocialPost` types</span>
...
<span class="caption">لیستینگ ۱۰-۱۴: Defining a `Summary` trait with a default implementation of the `summarize` method</span>
...
<span class="caption">لیستینگ ۱۰-۱۵: Conditionally implementing methods on a generic type depending on trait bounds</span>
